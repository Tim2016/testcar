...
?>
    ...
    <?php 
        //Adds
        if( $yog_shop_adds = yog_helper()->get_option( 'engines-shop-adds', 'raw', false, 'options' ) ){
            echo '<div class="row">
                    <div class="banner clearfix text-center">
                        <a href="'. yog_helper()->get_option( 'engines-shop-adds-link', 'url', false, 'options' ) .'"><img src="'. esc_url( $yog_shop_adds['url'] ) .'" alt="" class="img-responsive"></a>
                    </div>
                </div>';
        }
    ?>
