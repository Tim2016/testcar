...
class Spyropress_Widget_Products extends WC_Widget_Products {
	...
	public function widget( $args, $instance ) {
		...
		if ( ( $products = $this->get_products( $args, $instance ) ) && $products->have_posts() ) {
			...
			echo apply_filters( 'woocommerce_before_widget_product_list', '<table class="table"><tbody>' );
			...
			echo apply_filters( 'woocommerce_after_widget_product_list', '</tbody></table>' );
	 ...
	 public function widget( $args, $instance ) {
		...
		if ( $comments ) {
			...
			foreach ( (array) $comments as $comment ) {
				...
				echo '<tr><td class="product-img"><a href="' . esc_url( get_comment_link( $comment->comment_ID ) ) . '">'. $_product->get_image() .'</a></td>';

				echo '<td><a href="' . esc_url( get_comment_link( $comment->comment_ID ) ) . '"><strong>'. $_product->get_title() . '</strong></a>';

				echo $rating_html;

				printf( '<small class="reviewer">' . _x( 'by %1$s', 'by comment author', 'woocommerce' ) . '</small>', get_comment_author() );
...
class Spyropress_Widget_Price_Filter extends WC_Widget_Price_Filter {
	...
	public function widget( $args, $instance ) {
		...
		echo '<form method="get" action="' . esc_url( $form_action ) . '" class="product-section">
    			<div class="price_slider_wrapper price-filter ui-slider ui-slider-horizontal ui-widget ui-widget-content ui-corner-all">
    				<div class="price_slider" style="display:none;"></div>
    				<div class="price_slider_amount price-filter row">
    					<div class="price-input col-md-8">
                        ...
                        <div class="col-md-4"><button type="submit" class="button btn btn-primary pull-right filter">' . __( 'Filter', 'woocommerce' ) . '</button></div>
    					' . $fields . '
	...
	public function widget( $args, $instance ) {
		...
		if ( count( $_chosen_attributes ) > 0 || $min_price > 0 || $max_price > 0 ) {
			...
			if (!is_null($_chosen_attributes)){
				foreach ( $_chosen_attributes as $taxonomy => $data ) {
					...
					foreach ( $data['terms'] as $term_id ) {
						...
						echo '<tr><td class="chosen"><a class="btn btn-danger" title="' . esc_html__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . $term->name . '</a></td></tr>';
					}
				}
			}
			...
			if ( $min_price ) {
				...
				echo '<td class="chosen"><a class="btn btn-primary" title="' . esc_html__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . esc_html__( 'Min', 'woocommerce' ) . ' ' . wc_price( $min_price ) . '</a></td>';
			}

			if ( $max_price ) {
				$link = remove_query_arg( 'max_price' );
				echo '<td class="chosen"><a class="btn btn-primary" title="' . esc_html__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . esc_html__( 'Max', 'woocommerce' ) . ' ' . wc_price( $max_price ) . '</a></td>';
			}
...
register_widget( 'Spyropress_WC_Widget_Product_Categories' );