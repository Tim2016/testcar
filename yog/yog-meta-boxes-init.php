 ...
 * Theme Options Init
 *
 * @class Yog_Meta_Boxes initiate the theme meta fields option machine.
 */
		...
		foreach ( $sections as $section ) {
			...
			foreach ( $section['post_types'] as $post_type ) {
				...
				if ( isset( $section['separate_box'] ) && $section['separate_box'] === true ) {
					...
					$box_title = sprintf( esc_html__('%s Options', 'engines'), ucwords(str_replace( array('_','-'), ' ', $post_type ) ) );
				}
